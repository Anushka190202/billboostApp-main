    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        db.collection("clients")
            ...
            .addOnSuccessListener { result ->
                ...
                if (billModel.id.isNullOrEmpty() == false) {
                    var index =
                        clientList.indexOfFirst { element -> element.id == billModel.clientsModel?.id }
        ...
        db.collection("companies")
            ...
            .addOnSuccessListener { result ->
                ...
                if (billModel.id.isNullOrEmpty() == false) {
                    var index =
                        companyList.indexOfFirst { element -> element.id == billModel.companyModel?.id }
...
//                        billsItemAdapter.notifyDataSetChanged()
        ...
        fun calculateTotal() {
            ...
            binding.tvTotal.setText(
                (String.format(
                    "%.2f",
                    totalwithoutDeduction.toDouble()
                )).toString()
            )
